...
describe("GET /api/orcamentos", () => {
  ...
  it("filtra orçamentos do household (sem pessoa) com pessoaId=null", async () => {
    const { household, cookie } = await criarHouseholdComSessao();
    const categoria = await prismaTest.categoria.create({
      data: { nome: "Moradia", householdId: household.id },
    });
    ...
    const response = await GET(getRequest(cookie, "?ano=2026&pessoaId=null"));
    const body = await response.json();

    expect(response.status).toBe(200);
    expect(body).toHaveLength(1);
    expect(body[0].pessoaId).toBeNull();
  });
});
...
describe("POST /api/orcamentos", () => {
  ...
  it("retorna 400 para valor negativo", async () => {
    ...
    const response = await POST(
      postRequest(
        { categoriaId: categoria.id, ano: 2026, valorCentavos: -500 },
        cookie,
      ),
    );
  ...
  it("retorna 404 para categoria de outro household", async () => {
    ...
    const response = await POST(
      postRequest(
        { categoriaId: outraCategoria.id, ano: 2026, valorCentavos: 1000 },
        cookie,
      ),
    );
